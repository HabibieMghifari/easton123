...
def not_monster():
    for id, data in F14.monster.items():
        ada = False
        for idmonster, datamonster in F14.monster_shop.items():
            if id == idmonster:
                ada = True
                break
        if not ada:
...
def not_item():
    for id, data in F14.item.items():
        ada = False
        for iditem, dataitem in F14.item_shop.items():
            if id == iditem:
                ada = True
                break
        if not ada:
...
def tampilkan_shop(barang):
    if barang == 'monster':
        ...
        for id, data in F14.monster_shop.items():
            stok = data['stock']
            harga = data['price']
            for idmonster, datamonster in F14.monster.items():
                if idmonster == id:
        ...
        print("\nID | TYPE | STOK | HARGA")
        for id, data in F14.item_shop.items():
            type = data['type']
            stok = data['stock']
            harga = data['price']
    ...
    if F02.user['user_id'] == '0':
        ...
        if aksi == 'lihat':
            ...
            if sub_aksi == 'monster':
                ...
                for id, data in not_monster_shop.items():
                    if id_monster == id:
                        ...
                        F14.monster_shop.update(tambah)
            ...
            if sub_aksi == 'monster':
                ...
                if stok != '':
                    ubah_stok = True
                    F14.monster_shop[(id)]['stock'] = (stok)
                if harga != '':
                    ubah_harga = True
                    F14.monster_shop[(id)]['price'] = (harga)
                ...
                if stok != '':
                    ubah_stok = True
                    F14.item_shop[(id)]['stock'] = (stok)
                if harga != '':
                    ubah_harga = True
                    F14.item_shop[(id)]['price'] = (harga)
